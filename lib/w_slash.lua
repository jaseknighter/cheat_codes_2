...
function w_slash.wdel_add_params()
  ...
  params:add {
    ...
    action = function(val) 
      print("val",val,params:get("wdel_time_long"),val ~= w_slash.last_wdel_time)
      if w_slash.last_wdel_time and val ~= w_slash.last_wdel_time then
        print("new")
        crow.send("ii.wdel.time(" .. val .. ")") 
      end
      pset_wdel_time_long = val
      w_slash.last_wdel_time = val
      -- params:set("wdel_freeze",1)
    end
  }

  mc.DEL_FREQ_DEL = cs.def{
    min=0.1,
    max=2,
    warp='lin',
    step=0.1,
  ...
  params:add {
    ...
    id = "wdel_freq_del",
    name = "freq delay amt",
    controlspec = w_slash.DEL_FREQ_DEL,
    action = function(val) 
    end
  }
